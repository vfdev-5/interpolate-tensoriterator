...
void fmadd_avx(const float * x_ptr, const float * y_ptr, float * out_ptr, uint64_t size) {

    static constexpr uint32_t step = 32 / sizeof(float);

    uint64_t d = 0;
    __m256 vec_x, vec_y, vec_output;
    __m256 vec_const = _mm256_set1_ps(5.0f);
    for (; d < size - (size % step); d+=step) {        
        vec_x = _mm256_loadu_ps(x_ptr + d);
        vec_y = _mm256_loadu_ps(y_ptr + d);
        vec_output = _mm256_fmadd_ps(vec_x, vec_y, vec_const);
...
void fmadd_avx(const std::vector<float> & x, const std::vector<float> & y, std::vector<float> & out) {
    ...
    fmadd_avx(x_ptr, y_ptr, out_ptr, size);
}



void fmadd_non_vec(float * x_ptr, float * y_ptr, float * out_ptr, uint64_t size) {

    for (uint64_t i=0; i < size; i++) {
        out_ptr[i] = x_ptr[i] * y_ptr[i] + 5.0;
    }
}

void fmadd_non_vec(const std::vector<float> & x, std::vector<float> & y, std::vector<float> & out) {
...
/*
    make && ./bench_fma
    g++ -std=c++14 -mavx2 -O2 main_fma.cpp -o bench_fma
*/
int main(int argc, char** argv)
{
    ...
    unsigned int n = 10000;
    {
        ...
        {
            fmadd_non_vec(aligned_x, aligned_y, aligned_output, size);
            aligned_output[0]++;
        }
        auto end = std::chrono::steady_clock::now();        
        auto duration = end - timer;
        std::cout << "- fmadd_non_vec took " << duration.count() / n
    ...
    }
    {
        ...
        for (uint64_t j=0; j<n; ++j)
        {
            fmadd_non_vec(x, y, out);
            out[0]++;
        }
        auto end = std::chrono::steady_clock::now();        
        auto duration = end - timer;
        std::cout << "- fmadd_non_vec as std::vector took " << duration.count() / n
                  << " microseconds to complete." << std::endl;
    }
    {
        __attribute__ ((aligned (32))) float aligned_output[size];
        ...
        for (uint64_t j=0; j<n; ++j)
        {
            fmadd_avx(aligned_x, aligned_y, aligned_output, size);
            aligned_output[0]++;
        }
        auto end = std::chrono::steady_clock::now();
        auto duration = end - timer;
        std::cout << "- fmadd_avx took " << duration.count() / n
                  << " microseconds to complete." << std::endl;
    }
    {
        std::vector<float> out(size);
        ...
        auto end = std::chrono::steady_clock::now();
        auto duration = end - timer;
        std::cout << "- fmadd_avx as std::vector took " << duration.count() / n
                  << " microseconds to complete." << std::endl;
    }
    return 0;
}