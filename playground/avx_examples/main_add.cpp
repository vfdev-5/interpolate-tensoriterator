...
inline void sum_avx(const float * x_ptr, const float * y_ptr, float * out_ptr, uint64_t size) {
    ...
    for (; d < size - (size % step); d+=step) {        
        ...
        vec_output = _mm256_add_ps(vec_x, vec_y);
...
inline void sum_avx(const std::vector<float> & x, std::vector<float> & y, std::vector<float> & out) {
    static constexpr uint32_t step = 32 / sizeof(float);
    ...
    sum_avx(x_ptr, y_ptr, out_ptr, size);
}



inline void sum_non_vec(float * x_ptr, float * y_ptr, float * out_ptr, uint64_t size) {
...
inline void sum_non_vec(const std::vector<float> & x, std::vector<float> & y, std::vector<float> & out) {
...
int main(int argc, char** argv)
{
    ...
    }
    {
        ...
        for (uint64_t j=0; j<n; ++j)
        {
            sum_avx(aligned_x, aligned_y, aligned_output, size);
            aligned_output[0]++;
        }
        auto end = std::chrono::steady_clock::now();
        auto duration = end - timer;
